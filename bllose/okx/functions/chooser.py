...
def data_collection(instList: list) -> dict:
    """
    将原始数据转化为vo对象，并组装成key:value的形式
    Args:
        instList(list): 原始数据列表 from OKX
    Returns:
        candlerMap(dict): 组装好后的 vo Map
    """
    candlerMap = {}
...
candlerMap = data_collection(instList)
print(to_json(candlerMap))

btcCandler: CandleVo = candlerMap['BTC-USDT']
targetVol = btcCandler.volCcy

for _, curCandler in candlerMap.items():
